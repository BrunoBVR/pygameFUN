...
class Game:
    ...
    def load_data(self):
        ...
        self.player_img = pg.image.load(path.join(self.img_folder, PLAYER_IMG['pistol'])).convert_alpha()
        self.bullet_images = {}
    ...
    def update(self):
        ...
        for hit in hits:
            ...
            if hit.type == 'shotgun':
                ...
                self.player_img = pg.image.load(path.join(self.img_folder, PLAYER_IMG['shotgun'])).convert_alpha()
        # mobs hit player
        ...
        for mob in hits:
            # hit.health -= WEAPONS[self.player.weapon]['damage'] * len(hits[hit]) # Multiply by number of bullets hitting mob
    ...
    def draw(self):
        pg.display.set_caption("{:2f}".format(self.clock.get_fps())) # Track fps during dev
        ...
        # pg.draw.rect(self.screen, WHITE, self.player.hit_rect, 2) # Draw player rectangle
        if self.night:
            self.render_fog()
        # HUD functions
    ...
    def show_go_screen(self):
        ...
        self.draw_text("Press a key to start a new game!", self.title_font,
                       50, WHITE, WIDTH/2, HEIGHT * 3/4, align="center")
        pg.display.flip()
        self.wait_for_key()
